...
df <- df  %>%
  filter(initial_use %in% initial_uses & final_use %in% final_uses) %>%
...
for (frr in frrs) {
  ...
  saveRDS(frr_list[[frr]], paste("processing/ccp/frr_results/", frr, ".rds", sep=""))
}  

#Merge the datasets
state_files  <- list.files(path = "processing/ccp/state_results/", pattern = '\\.csv')
setwd("processing/ccp/state_results/")
...
write.csv(result_acres, "processing/ccp/ccps.csv") # write csv
...
ccps <- read_csv("processing/ccp/ccps.csv")
...
summary(ccps_check$total_ccp)

TEMP <- ccps_check %>%
  filter(fips == 13015, year == 2002, lcc == "7_8", initial_use == "CRP") # data from state and FRR conflicts with each other

TEMP2 <- ccps_check %>%
  filter(total_ccp > 1.05)
