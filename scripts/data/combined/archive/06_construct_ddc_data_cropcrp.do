...
replace weighted_ccp = weighted_ccp*(5/3) if initial_use != final_use & year == 2015 & weighted_ccp < 3/5
bysort fips lcc initial_use: egen prob_out = sum(weighted_ccp) if year == 2015 & initial_use != final_use 
bysort fips lcc initial_use: egen max_prob_out = max(weighted_ccp) if year == 2015
replace weighted_ccp = 1 - max_prob_out if year == 2015 & initial_use == final_use
...
/* Keep only crop and CRP       	   										*/ 
keep if initial_use == "CRP" | initial_use == "Crop"
keep if final_use == "CRP" | final_use == "Crop"
...
foreach use in crop CRP {
	forvalues i = 0/4 {
		gen `use'Xlcc`i' = lcc`i'*`use'_nr
		}
	}
	
save "processing\combined\ddc_data_cropcrp", replace
