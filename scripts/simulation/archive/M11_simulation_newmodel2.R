...
df <- read.csv(sprintf("%s/sim_df_crprev.csv",input_path)) %>% as.data.table() %>% 
        mutate(fips = ifelse(fips == 12025, 12086, fips)) #Fix Miami-Dade fips code
        # %>%
        # mutate(fips = str_pad(fips, width = 5, side = "left", pad = "0"))
        # %>% 
...
est <- "results\\initial_estimation\\regs_2022-07\\regs_2022-07-12_lcc_st_newmodel2.est"
...
sim2 <- simulate_luc(returns = mod_returns(rtns, forest_fn = function(x) 1.2*x), estimates = est)

dst <- "processing/simulation/simulated_probs/results_22-27-07/"
dir.create(dst, recursive = TRUE, showWarnings = FALSE)
write.csv(baseline, paste0(dst,"baseline.csv"))
write.csv(sim, paste0(dst,"sim_crop_returns_1.2.csv"))
write.csv(sim2, paste0(dst,"sim_forest_returns_1.2.csv"))
