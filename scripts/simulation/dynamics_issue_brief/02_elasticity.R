...
simulate_luc <- function(df_input, returns, estimates, pfactor, conv.type) {
  ...
  df.p <- df.out %>% select(fips,lcc, ecoregion, year, 
            ...
            mutate(p_kj = ifelse(final_use == conv.type & initial_use != conv.type, phat, 0),
                   ...
                   pnew = ifelse(final_use != conv.type, phat*(1-p_kj_prime)/(1-p_kj), pnew))   %>% 
...
status_quo <- run_sim(df, 
                 ...
                 carbon_path = sprintf("%statu_quo/carbon_model_input/", output_path))
save_county_acreages(status_quo, sprintf("%status_quo/", output_path))
...
urban <- run_sim(df, 
                ...
                carbon_path = sprintf("%surbanization_decline/carbon_model_input/", output_path))

save_county_acreages(urban, sprintf("%surbanization_decline/", output_path))
...
forest <- run_sim(df, 
                 ...
                 carbon_path = sprintf("%sforest_increase/carbon_model_input/", output_path))

save_county_acreages(forest, sprintf("%sforest_increase/", output_path))
