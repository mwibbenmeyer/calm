...
# Actual conversions
ddc_data_cropcrp <- read_dta("processing/combined/ddc_data_cropcrp.dta") %>%
  filter(is.na(CRP_nr)==FALSE, is.na(crop_nr)==FALSE) 

sum(distinct(ddc_data_cropcrp,fips,year,lcc,initial_use,.keep_all=TRUE)$initial_acres)
sum(ddc_data_cropcrp$final_acres)

initial <- ddc_data_cropcrp %>%
  distinct(fips,year,lcc,initial_use,.keep_all=TRUE) %>%
  group_by(year, initial_use) %>%
  summarize(initial_acres = sum(initial_acres))

write_csv(initial, "processing/elasticity/initial_acres.csv")

initial_county <- ddc_data_cropcrp %>%
  distinct(fips,year,lcc,initial_use,.keep_all=TRUE) %>%
  group_by(fips, year, initial_use) %>%
  summarize(initial_acres = sum(initial_acres))

write_csv(initial_county, "processing/elasticity/initial_acres_county.csv")

initial_county_lcc <- ddc_data_cropcrp %>%
  distinct(fips,year,lcc,initial_use,.keep_all=TRUE) %>%
  group_by(fips, year, initial_use, lcc) %>%
  summarize(initial_acres = sum(initial_acres))

write_csv(initial_county_lcc, "processing/elasticity/initial_acres_county_lcc.csv")

final <- ddc_data_cropcrp %>%
...
conversions_alt <- ddc_data_cropcrp %>%
