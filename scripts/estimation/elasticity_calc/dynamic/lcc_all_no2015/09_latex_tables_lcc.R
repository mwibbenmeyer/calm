...
## Read in data
final_acres_lcc <- read_excel("processing/elasticity/lcc_all_no2015/final_acres_lcc.xlsx")
...
## Export the table
dst = 'results/model_results/elasticities/lcc_all_no2015/'
...
## Read in data
reg_lcc <- read_delim("results/initial_estimation/regs_2022-03/regs_2022-03-01_unweighted_lcc_3digits_dy_no2015.txt", 
...
cost_coef <- reg_lcc %>% # better to use 4 digits to get more accurate results after multiplying estimates by 10
  slice(41:152) %>%
  mutate(id = row_number(),
         var2 = var) %>%
...
cost_coef_x10 <- cost_coef %>%
  mutate(Crop_num = round(parse_number(Crop)*10,3),
         CRP_num = round(parse_number(CRP)*10,3),
         Forest_num = round(parse_number(Forest)*10,3),
         Urban_num = round(parse_number(Urban)*10,3),
         Others_num = round(parse_number(Others)*10,3)) %>%
  mutate(id = row_number()) %>%
  mutate(Crop_final = case_when(id%%2==1 ~ paste(sprintf('%.3f',Crop_num), str_replace_all(Crop, as.character(sprintf('%.3f',Crop_num/10)), ""), sep=""),
                                id%%2==0 ~ paste("(", sprintf('%.3f',Crop_num), ")", sep="")),
         CRP_final = case_when(id%%2==1 ~ paste(sprintf('%.3f',CRP_num), str_replace_all(CRP, as.character(sprintf('%.3f',CRP_num/10)), ""), sep=""),
                                id%%2==0 ~ paste("(", sprintf('%.3f',CRP_num), ")", sep="")),
         Forest_final = case_when(id%%2==1 ~ paste(sprintf('%.3f',Forest_num), str_replace_all(Forest, as.character(sprintf('%.3f',Forest_num/10)), ""), sep=""),
                                id%%2==0 ~ paste("(", sprintf('%.3f',Forest_num), ")", sep="")),
         Urban_final = case_when(id%%2==1 ~ paste(sprintf('%.3f',Urban_num), str_replace_all(Urban, as.character(sprintf('%.3f',Urban_num/10)), ""), sep=""),
                                id%%2==0 ~ paste("(", sprintf('%.3f',Urban_num), ")", sep="")),
         Others_final = case_when(id%%2==1 ~ paste(sprintf('%.3f',Others_num), str_replace_all(Others, as.character(sprintf('%.3f',Others_num/10)), ""), sep=""),
                                id%%2==0 ~ paste("(", sprintf('%.3f',Others_num), ")", sep=""))) %>%
  mutate(Crop = case_when(is.na(Crop_num)==TRUE ~ "",
                          TRUE ~ Crop_final),
         CRP = case_when(is.na(CRP_num)==TRUE ~ "",
                           TRUE ~ CRP_final),
         Forest = case_when(is.na(Forest_num)==TRUE ~ "",
                          TRUE ~ Forest_final),
         Urban = case_when(is.na(Urban_num)==TRUE ~ "",
                          TRUE ~ Urban_final),
         Others = case_when(is.na(Others_num)==TRUE ~ "",
                          TRUE ~ Others_final)) %>%
  select(initialuse, flag, Crop, CRP, Forest, Urban, Others)

cost_coef_formatted <- cost_coef_x10 %>%
  mutate(id = row_number()) %>%
  mutate(LCC = case_when(id%%2==0 ~ "", 
                         flag=="LCC1_par1" ~ "\\multirow{2}{*}{1--2}",
                         flag=="LCC2_par1" ~ "\\multirow{2}{*}{3--4}",
...
## Export the tables
dst = 'results/model_results/regressions/lcc_all_no2015/'
dir.create(dst, recursive = TRUE, showWarnings = FALSE)
