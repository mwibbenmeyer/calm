...
* Drop y from 2012 (which are based on 2015 ccps)
drop if year == 2012
tab year if y != .
...
// capture mkdir `dir'
// log using "`dir'regs_2022-03-29_unweighted_lcc_dy_no2015.log", replace
// capture rm "`dir'regs_2022-03-29_unweighted_lcc_dy_no2015.xls" 
// capture rm "`dir'regs_2022-03-29_unweighted_lcc_dy_no2015.txt"
// capture rm "`dir'regs_2022-03-29_unweighted_lcc_3digits_dy_no2015.xls"
// capture rm "`dir'regs_2022-03-29_unweighted_lcc_3digits_dy_no2015.txt"
...
// outreg2 using "`dir'regs_2022-03-29_unweighted_lcc_dy_no2015.xls", e(r2_a) stats(coef se) bdec(10) ///
// addtext(weights, NO) append
// outreg2 using "`dir'regs_2022-03-29_unweighted_lcc_3digits_dy_no2015.xls", e(r2_a) stats(coef se) bdec(3) ///
// addtext(weights, NO) append
...
tab year lcc if e(sample)==1
bysort year: tab initial_use final_use if e(sample)==1
bysort year: tab initial_use final_use if e(sample)==1 & initial_acres != final_acres //actual conversions
bysort lcc: tab initial_use final_use if e(sample)==1 & initial_acres != final_acres //actual conversions

global beta = 0.9
gen y_dy = ln(weighted_ccp/pkk) + $beta * ln(pjj/pjk)
/*Code y as missing for observations in which there is no land in initial use in county-LCC in year t*/
replace y_dy = . if initial_acres == 0
// gen y_diff = abs(y2-y)
// sum y_diff, d // all match

global beta = 0
gen y_st = ln(weighted_ccp/pkk) + $beta * ln(pjj/pjk)
/*Code y as missing for observations in which there is no land in initial use in county-LCC in year t*/
replace y_st = . if initial_acres == 0
// sum y_st if y_dy==. // no obs

keep if e(sample)==1
save "processing\combined\reg_diagnostics"
