...
simulate_luc <- function(returns, estimates) {
  ...
  qui do "scripts/simulation/merge_crp_and_others/program_calc_phats_lcc_newmodel2.do"
...
df <- read.csv(sprintf("%s/sim_df_crprev_urbannrsub.csv",input_path)) %>% as.data.table() 
        # %>%
        # mutate(fips = str_pad(fips, width = 5, side = "left", pad = "0"))
        # %>% 
...
rtns <- read.csv(sprintf("%s/returns_crprev_urbannrsub.csv",input_path)) %>% select(-X)

# Path of estimates file
est <- "results\\initial_estimation\\regs_2022-07\\regs_2022-07-12_lcc_st_replace_urban.est"
...
crop_sim = simulate_luc(returns = mod_returns(rtns, crop_fn = function(x) 1.2*x), estimates = est)
crop_sim = merge(baseline, crop_sim, by = c("landuse", "initial_acres"), suffixes = c("_base","_sim")) %>%
  mutate(base_change = final_acres_base - initial_acres, sim_change = final_acres_sim - initial_acres,
         perc_diff_change = (abs(sim_change) - abs(base_change)) / base_change * 100,
         perc_diff_change_per_rtn_perc_change = perc_diff_change / 20)

forest_sim = simulate_luc(returns = mod_returns(rtns, forest_fn = function(x) 1.2*x), estimates = est) 
forest_sim = merge(baseline, forest_sim, by = c("landuse", "initial_acres"), suffixes = c("_base","_sim")) %>%
  mutate(base_change = final_acres_base - initial_acres, sim_change = final_acres_sim - initial_acres,
         perc_diff_change = (abs(sim_change) - abs(base_change)) / base_change * 100,
         perc_diff_change_per_rtn_perc_change = perc_diff_change / 20)

urban_sim = simulate_luc(returns = mod_returns(rtns, urban_fn = function(x) 1.2*x), estimates = est) 
urban_sim = merge(baseline, urban_sim, by = c("landuse", "initial_acres"), suffixes = c("_base","_sim")) %>%
  mutate(base_change = final_acres_base - initial_acres, sim_change = final_acres_sim - initial_acres,
         perc_diff_change = (abs(sim_change) - abs(base_change)) / base_change * 100,
         perc_diff_change_per_rtn_perc_change = perc_diff_change / 20)

other_sim = simulate_luc(returns = mod_returns(rtns, other_fn = function(x) 1.2*x), estimates = est) 
other_sim = merge(baseline, other_sim, by = c("landuse", "initial_acres"), suffixes = c("_base","_sim")) %>%
  mutate(base_change = final_acres_base - initial_acres, sim_change = final_acres_sim - initial_acres,
         perc_diff_change = (abs(sim_change) - abs(base_change)) / base_change * 100,
         perc_diff_change_per_rtn_perc_change = perc_diff_change / 20)
