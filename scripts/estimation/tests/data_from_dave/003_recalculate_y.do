...
capture drop pkk pjj pjk y_dy y_st
...
global beta = 0
// gen y_st = ln(weighted_ccp/pkk) + $beta * ln(pjj/pjk) // to make sure y_st and y_dy have the same number of non-missing values
gen y_st = ln(weighted_ccp/pkk)
...
drop if y_st == . // make sure the sample remains the same when switching datasets

save "processing\combined\ddc_data_dave", replace
