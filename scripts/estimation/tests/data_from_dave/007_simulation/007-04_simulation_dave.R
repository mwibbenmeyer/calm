...
source("scripts/estimation/tests/data_from_dave/007_simulation/007-00_fn_modify_returns_dave.R")
...
simulate_luc <- function(returns, estimates) {
  ...
  qui do "scripts/estimation/tests/data_from_dave/007_simulation/007-00_program_calc_phats_dave.do"
  ...
  df.p <- df.out %>% select(fips,lcc,initial_use,final_use,phat) %>% 
    ...
    select(-final_use) %>% 
    mutate(across(pUrban:pForest, ~ .x*acres, .names = "a{substr(col,2,nchar(col))}"))
                                          
  df.l <- df.p %>% 
    mutate(across(aUrban:aForest, ~ .x, .names = "{substr(col,2,nchar(col))}")) %>% #Rename columns prior to pivot long
                    mutate(initial_acres = acres) %>% 
                    pivot_longer(cols = c('Urban','Crop','Forest'),
                                names_to = "final_use",
                                names_repair = "unique",
...
df <- read.csv(sprintf("%s/sim_df_dave.csv",input_path)) %>% as.data.table() 
        # %>% 
...
rtns <- read.csv(sprintf("%s/returns_dave.csv",input_path)) %>% select(-X)

# Path of estimates file
est <- "results/initial_estimation/regs_2022-05/regs_2022-05-19_unweighted_nolcc_st_southeast_newdata.est"
...
simulate_luc(returns = mod_returns(rtns, crop_fn = function(x) 1.5*x), estimates = est)
simulate_luc(returns = mod_returns(rtns, forest_fn = function(x) 1.5*x), estimates = est)
simulate_luc(returns = mod_returns(rtns, urban_fn = function(x) 1.5*x), estimates = est)
