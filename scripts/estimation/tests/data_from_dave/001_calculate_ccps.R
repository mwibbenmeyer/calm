...
initial_uses <- c("Crop","Forest","Urban")
final_uses <- c("Crop","Forest","Urban")
df <- df  %>%
  filter(initial_use %in% initial_uses,
         final_use %in% final_uses) %>%
  filter(!(initial_use == "Urban" & final_use != "Urban")) %>%
  .[ , .(total_acres = acresk), by = c("fips","year","initial_use","final_use","lcc")] %>%
                                                                        ...
                                                                        lapply(years, function(y)
                                                                          lapply(lcc_values, function(l)
                                                                            lapply(initial_uses, function(i)
                                                                              lapply(final_uses, function(f) smooth_ccps_state(state = state, yr = y, lcc_value = l, initial = i, final = f)))))))))
  write.csv(state_list[[state]], paste("processing/ccp/state_results_cfu/", state, ".csv", sep=""), row.names = F, na="")
  saveRDS(state_list[[state]], paste("processing/ccp/state_results_cfu/", state, ".rds", sep=""))
}
...
for (frr in frrs) {
  frr_list[[frr]] <- do.call(rbind, do.call(rbind, do.call(rbind, do.call(rbind,  #Row bind to unnest results
                                                                         lapply(years, function(y)
                                                                           lapply(lcc_values, function(l)
                                                                             lapply(initial_uses, function(i)
                                                                               lapply(final_uses, function(f) smooth_ccps_frr(frr = frr, yr = y, lcc_value = l, initial = i, final = f)))))))))
  write.csv(frr_list[[frr]], paste("processing/ccp/frr_results_cfu/", frr, ".csv", sep=""), row.names = F, na="")
  saveRDS(frr_list[[frr]], paste("processing/ccp/frr_results_cfu/", frr, ".rds", sep=""))
}  

#Merge the datasets
state_files  <- list.files(path = "processing/ccp/state_results_cfu/", pattern = '\\.csv')
setwd("processing/ccp/state_results_cfu/")
state_tables <- lapply(state_files, read.csv, header = TRUE)
result_state <- do.call(rbind , state_tables)

setwd("../frr_results_cfu/")
frr_files  <- list.files(pattern = '\\.csv')
frr_tables <- lapply(frr_files, read.csv, header = TRUE)
result_frr <- do.call(rbind , frr_tables)

setwd("../../../")
getwd()

# Add original and smoothed CCPs together and label the data source ------------
# Priority list: state results not 0 or 1 > frr results not 0 or 1 > state results not NA > frr results not NA
...
result_acres <- df2 %>%
  select(-resource_region) %>%
  mutate(fips = as.numeric(fips)) %>%
  right_join(result, by=c("fips", "year", "lcc", "initial_use", "final_use"))

write.csv(result_acres, "processing/ccp/ccps_cfu.csv") # write csv

# temp <- result_acres %>%
#   group_by(year, lcc, initial_use, final_use) %>%
#   summarize(avg_ccp = round(mean(weighted_ccp),3))
# write.csv(temp, "processing/ccp/ccps_summary.csv")

temp <- result_acres %>%
  ungroup() %>%
  group_by(year, fips, lcc, initial_use) %>%
  mutate(total_ccp = sum(weighted_ccp))

summary(temp$total_ccp)
