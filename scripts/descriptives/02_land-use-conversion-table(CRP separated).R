...
setwd("L:/Project-Land_Use/")

points <- read_dta("processing_output/pointpanel_estimation_unb.dta") %>% as.data.table()

points <- points[ , c("initial_use","final_use") := .(recode(initial_use,"Pasture" = "Other",
                                                                         "Range" = "Other"),
                                                      recode(final_use, "Pasture" = "Other",
                                                             "Range" = "Other"))] %>%
...
order <- data.table(initial_use=c("Crop","Forest","Urban","CRP","Other"), order=c(1,2,3,4,5))
