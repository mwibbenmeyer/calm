...
conversion_rates <- function(region) {
  
  df.init <- df[ecoregion == region & year >= 2002 & year <= 2015] %>% 
    .[ , .(initial_acres = sum(acresk)) , by = c("initial_use","year")]
  
  df.conv <- df[ecoregion == region & year >= 2002] %>% 
...
area_change <- function(df, conv.rates) {
  ...
  out <- df.conv[ , final_acres := mean_prob*initial_acres] %>% 
              .[ , .(initial_acres = sum(final_acres, na.rm = T)), by = c("final_use","ecoregion","year")]
  
  }

calc_land_use_change <- function(region, timesteps) {
  
  conv.rates <- conversion_rates(region)
  
  # Set up data set
  df.init <- df[ecoregion == region & year == 2015] %>% 
    .[ , .(initial_acres = sum(acresk, na.rm = T)), by = c("final_use", "ecoregion","year")]  
...
calc_hist_land_use <- function(region) {
  
  df.hist <- df[ecoregion == region & year >= 1987 & year <= 2015, 
...
study_regions <- unique(df[fips %in% study.counties]$ecoregion)


final.df <- rbind(do.call(rbind, lapply(study_regions, function(c) calc_land_use_change(c,10))),
                  do.call(rbind, lapply(study_regions, function(c) calc_hist_land_use(c)))) %>% 
              arrange(ecoregion,year,final_use)
dst <- "processing/misc/aflac/"
dir.create(dst, recursive = T, showWarnings = F)
write.csv(final.df, paste0(dst,"study_ecoregion_landuse_change.csv"))
