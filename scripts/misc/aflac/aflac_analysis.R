...
conversion_rates <- function(countyfips) {
  ...
  df.init <- df[ecoregion == ecoregions[fips == countyfips]$ecoregion & year >= 2002 & year <= 2015] %>% 
    .[ , .(initial_acres = sum(acresk)) , by = c("initial_use","year")]
  
  df.conv <- df[ecoregion == ecoregions[fips == countyfips]$ecoregion & year >= 2002] %>% 
...
write.csv(final.df, paste0(dst,"study_county_landuse_change.csv"))
