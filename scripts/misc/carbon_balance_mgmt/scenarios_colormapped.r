...
ecoregion = read_csv("raw_data/misc/ecoregions/interpolated_ecocd_counties.csv")
...
counties_sf <- get_acs(state = states, geography = "county", year = 2010, variables = "B19013_001", geometry = TRUE) %>%
  select(-c("variable","estimate","moe")) %>%
  st_transform(aea) %>% 
  st_make_valid()
mapping_data = merge(x = counties_sf, y = ecoregion, by.x = 'GEOID', by.y = 'fips')
...
colormap = mapping_data %>%
  ...
  scale_fill_manual(name = "Ecoregions",
                    ...
                    labels=c(
                              ...
                              "251" = "Prarie Parkland-Temp.",
                              "255" = "Prarie Parkland-Subtropical",
                              "411" = "Everglades", #Missing from Dave
                              
                              #Great Plains
                              "331" = "S. Rockies/Great Plains",
                              "332" = "Middle Rockies",
                              "315" = "S.W. Plateau",
                              # "311" = "#d94701",
                              ...
                              "262" = "Calif. Dry Steppe",
                              "263" = "Calif. Coastal Forest" 
                              ), na.value="black") +
    theme_minimal()
colormap

ggsave('results/misc/carbon_balance_mgmt/colormap_ecoregions.png', width = 6.5*1.4, height = 3*1.4)
