...
rents <- read.csv("raw_data/net_returns/forest/forest_plot_rents/plot_rents_by_county_South.csv") %>% 
...
# Import data and restrict to counties in forest returns sample 
points <- read_dta("processing/pointpanel/countypanel_estimation_bal.dta") %>% as.data.table() %>% 
            .[fips %in% unique(rents$county)]
